var roundNumber;
var trackRounds = 0;
let compNames = ["Jan", "Bilal", "Mohammed", "Kennedi", "Yassine"];
...
function login() {
    ...
    if (/\w{2,10}/.test(player) == false || /\d/.test(roundNumber) == false) {
        alert(
            "Voer een geldige naam van minimaal 2 letters in, vergeet het aantal rondes niet."
        );
        return 0;
    } else if (roundNumber > 10 || roundNumber < 3) {
        alert("Voer een getal in tussen 3 en 10.");
        return 0;
    }
            ...
            if (compDisplay.innerText == playerDisplay.innerText) {
                ...
                switch (choices) {
                    case "👊🖐":
                        ...
                        trackCompScore += 1;
                        comments.innerText = "Verloren❕";
                        break;
                    case "👊✌":
                        playerScore.innerText = trackPlayerScore + 1;
                        trackPlayerScore += 1;
                        comments.innerText = "Gewonnen❕";
                        break;
                    case "🖐✌":
                        compScore.innerText = trackCompScore + 1;
                        trackCompScore += 1;
                    ...
                    case "✌👊":
                        compScore.innerText = trackCompScore + 1;
                        trackCompScore += 1;
                        comments.innerText = "Goed gedaan";
                        break;
                }
            }
        }
